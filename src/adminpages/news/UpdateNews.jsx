...
import { FaArrowLeft, FaCamera, FaSave, FaHeading, FaAlignLeft, FaImage } from 'react-icons/fa';
...
const UpdateNews = () => {
        ...
        const fetchNewsData = async () => {
            try {
                ...
                if (item) {
                    ...
                    setMessage({ text: 'Không tìm thấy thông tin bài viết.', type: 'error' });
                }
            } catch (error) {
                console.error('Error fetching news:', error);
                setMessage({ text: 'Lỗi khi tải dữ liệu tin tức.', type: 'error' });
            } finally {
                setFetching(false);
            }
        };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const data = await response.json();
            if (data.success) {
                setMessage({ text: 'Cập nhật tin tức thành công!', type: 'success' });
                setTimeout(() => {
                    navigate('/admin/news');
                }, 1500);
            } else {
    ...
    return (
        <div className="p-8 max-w-5xl mx-auto min-h-screen">
            ...
            <div className="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none mb-2 uppercase">CHỈNH SỬA TIN TỨC</h1>
                    <p className="text-sm font-medium text-slate-500">Cập nhật nội dung truyền thông cho bài viết tin tức.</p>
                </div>
            </div>
